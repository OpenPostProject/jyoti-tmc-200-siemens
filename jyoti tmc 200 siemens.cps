var modelType = "TMC 200";
description = "Jyoti TMC-200 with Siemens control";
...
function getmachinetype() {
  if (modelType == "AX 200-M") {
    ...
    gotPolarInterpolation = true; // specifies if the machine has XY polar interpolation (TRANSMIT) capabilities
    gotSecondarySpindle = false;
  } else if (modelType == "AX 200-MY") {
    gotYAxis = true;
    yAxisMinimum = toPreciseUnit(gotYAxis ? -40.0 : 0, MM); // specifies the minimum range for the Y-axis
    yAxisMaximum = toPreciseUnit(gotYAxis ? 40.0 : 0, MM); // specifies the maximum range for the Y-axis
    gotBAxis = false; // B-axis always requires customization to match the machine specific functions for doing rotations
    gotMultiTurret = false; // specifies if the machine has several turrets
    gotPolarInterpolation = true; // specifies if the machine has XY polar interpolation (TRANSMIT) capabilities
    gotSecondarySpindle = false;
...
function writeRetract() {
  ...
  for (var i = 0; i < arguments.length; ++i) {
    ...
    case Z:
      zOutput.reset();
      words.push(zOutput.format((currentSection.spindle == SPINDLE_SECONDARY) ? getProperty("g75HomePositionSubZ") : getProperty("homePositionZ")));
      retracted = true; // specifies that the tool has been retracted to the safe plane
      break;
    default:
...
function onOpen() {
  ...
  // writeBlock("#" + (firstFeedParameter - 1) + "=" + ((currentSection.spindle == SPINDLE_SECONDARY) ? getProperty("g75HomePositionSubZ") : getProperty("homePositionZ")), formatComment("homePositionZ"));
...
function getFeed(f) {
  ...
  if (machineState.feedPerRevolution) {
    _f = (feedFormat.format(_f) <= 0) ? (Math.pow(10, feedFormat.getNumberOfDecimals() * -1)) : _f;
  }
...
function onRapid(_x, _y, _z) {
  ...
  if (x || y || z) {
    ...
    if (pendingRadiusCompensation >= 0) {
      ...
      writeBlock(gMotionModal.format(0), x, y, z); // G1 required for cutter compensation moves only
    }
  }
}
...
function onLinear(_x, _y, _z, feed) {
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      switch (radiusCompensation) {
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(gMotionModal.format(isSpeedFeedSynchronizationActive() ? 32 : 1), gFormat.format(ccLeft), x, y, z, f);
        break;
      case RADIUS_COMPENSATION_RIGHT:
        writeBlock(gMotionModal.format(isSpeedFeedSynchronizationActive() ? 32 : 1), gFormat.format(ccRight), x, y, z, f);
        break;
      default:
    ...
    case PLANE_ZX:
      if (!isSpiral()) {
        ...
        if (!cFormat.areDifferent(c, cOutput.getCurrent())) {
          ...
          var start = getCurrentPosition();
          writeBlock(gMotionModal.format(directionCode), xOutput.format(getModulus(x, y)), cOutput.format(c), zOutput.format(z), iOutput.format(cx - start.x, 0), kOutput.format(cz - start.z, 0), getFeed(feed));
          previousABC.setZ(c);
          return;
        }
      }
